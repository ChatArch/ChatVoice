...
def database_settings() -> dict[str, object]:
    ...
    ChatVoice v0.1.5 ships the Speakr storage layer on SQLite WAL for a single
    ...
    return {
        ...
        "note": (
            ...
            if supported
            else "External DB URL detected; provider/API architecture supports this deployment direction, but v0.1.5 web storage still needs the SQL repository migration before use."
        ),
    }
