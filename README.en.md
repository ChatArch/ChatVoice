...
python -m pip install "ChatVoice[web]==0.1.8"
...
v0.1.8 defaults to SQLite WAL under:

```text
<chatarch-home>/chatvoice/data/meetings.sqlite3
```

Use one service process with SQLite (`--workers 1`). High-concurrency production should migrate the storage layer to Postgres/MySQL before scaling workers or nodes. An external database URL setting is detected in `chatvoice doctor` / `chatvoice service plan`, but the v0.1.8 packaged legacy storage layer still supports SQLite only.
