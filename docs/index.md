...
- **部署与启动**
    ...
    从 `python -m pip install "ChatVoice[web]==0.1.15.post3"` 到 `chatvoice serve app`，说明运行目录、账号创建、ASR API provider、数据库并发边界。此版本为尚未发布的本地 hotfix；系统语音配置见[独立 TTS](tts-models.md)。
...
- v0.1.15 默认 SQLite WAL，适合单服务进程轻并发；高并发数据库迁移需单独版本。
