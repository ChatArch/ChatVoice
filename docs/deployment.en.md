...
This page explains how to run a ChatVoice / Speakr service from the released Python package in v0.1.13: install, create an account, start the service, generate an API token, and read meeting/summary data.
...
The recommended production shape in v0.1.13 is **ChatVoice calls ASR through an API provider**. That provider can be:
...
The v0.1.13 packaged web app uses SQLite WAL by default:
