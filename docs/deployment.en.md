...
This page explains how to run a ChatVoice / Speakr service from the released Python package in v0.1.8: install, create an account, start the service, generate an API token, and read meeting/summary data.
...
The recommended production shape in v0.1.8 is **ChatVoice calls ASR through an API provider**. That provider can be:
...
The v0.1.8 packaged web app uses SQLite WAL by default:
...
- an external database URL setting is detected by `doctor` / `service plan`, but the v0.1.8 packaged legacy storage layer still supports SQLite only.
