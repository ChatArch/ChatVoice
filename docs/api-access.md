...
ChatVoice 0.1.0 增加了从 packaged service 读取会议和对话数据的闭环：先用受邀账号登录，再生成 API Token，最后用 bearer token 调 `/api/data/...` 或 `chatvoice data ...`。
...
- Token 被撤销或过期后立即不可用。
- 数据读取接口会返回转写文本和摘要内容；不要把输出贴入公共日志或 PR。
