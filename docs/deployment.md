...
这一页说明 v0.1.7 发布后，如何只通过 Python 包搭起一套 ChatVoice / Speakr 服务流程：安装、创建账号、启动服务、生成 API Token、读取会议/摘要数据。
...
v0.1.7 的生产推荐方式是 **ChatVoice 后端通过 API 调用 ASR 服务**。这个 ASR 服务可以是：
...
v0.1.7 packaged Web app 默认使用 SQLite WAL：
...
- an external database URL setting 会被 `doctor` / `service plan` 检测并报告，但 v0.1.7 的 packaged legacy storage 仍只真正支持 SQLite。
