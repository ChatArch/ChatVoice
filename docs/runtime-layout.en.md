...
This page documents where `pip install "ChatVoice[web]==0.1.5"` installs code, where runtime data is written by default, and which data lives in SQLite versus the browser.
...
The `0.1.5` packaged legacy storage still supports SQLite WAL only. It is suitable for one service process, light concurrency, and controlled internal use:
