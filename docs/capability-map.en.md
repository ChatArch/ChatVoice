...
| Postgres/MySQL storage | Not implemented | External URLs are detected in plan/doctor, but v0.1.9 packaged legacy storage still supports SQLite only. |

## Out of scope now

- Do not bundle GPU model download, CUDA/PyTorch installation, and the web process as one default runtime.
- Do not claim MySQL/Postgres is complete in v0.1.9; high-concurrency storage migration needs a separate release.
