...
- 直接选择 ChatLogin 0.1.2 内建的 ChatVoiceAuth 后端；宿主只保留连接、时钟、HTTP 响应与业务权限映射，删除重复认证存储实现。
- `/login` 复用 LoginUI 的表单、脚本与样式，并提供声笺品牌覆盖和 Python 自定义入口；原账号/会话、Cookie、受邀注册和 API Token 权限保持兼容。
- 登录前保存访客草稿，录音或实时对话进行中拒绝离开；访客回流不自动上传本地记录。
